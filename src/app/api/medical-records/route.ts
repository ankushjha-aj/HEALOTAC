...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const [newRecord] = await db.insert(medicalRecords).values({
      ...
      exPpg: exPpg ? parseInt(exPpg) : 0,
      attendB: attendB ? parseInt(attendB) : 0,
